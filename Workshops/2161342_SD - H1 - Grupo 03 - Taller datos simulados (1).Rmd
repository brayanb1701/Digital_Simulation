...
cantInteracciones<-function(errMax=6/60, results=resultadosExper(experimentos())){
  m <- results$m
  s<- results$s
  tesp<-results$datos
  k<- length(tesp)
...
cantInteraInterv<-function(errMax=5/60, results=resultadosExper(experimentos()), interConf=0.97){
  m <- results$m
  s<- results$s
  tesp<-results$datos
  alpha<-1-interConf
...
sprintf("Varianza esperada: %f", mediaVan$m)
...
* Tiempo de tiempo de espera del cliente antes de irse (tp)
...
Para el intervalo de confianza dado (90%) tenemos que nuestro $\alpha/2 = 0.005$ y nuestro $z_{\alpha/2}=2.576$, ahora procedemos a calcular el margen de error o error máximo de la siguiente manera:
```{r}
